    ...
    <div v-else class="grid gap-3">
      <div
        ...
        :class="[
          ...
          isPlanSelected(plan)
            ? (themeStore.isDark
              ? 'border-[var(--border-color)] bg-[var(--bg-surface-soft)]'
              : 'border-[var(--border-color)] bg-[var(--bg-surface-soft)]')
            : isPlanDisabled(plan)
      ...
      >
        ...
        <div
          ...
          class="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-[var(--text-primary)] flex items-center justify-center shadow-lg"
        >
          <svg class="w-4 h-4 text-[var(--bg-surface-soft)]" fill="currentColor" viewBox="0 0 20 20">
        ...
        <!-- 资源配置区域 - 分组展示（透明，跟随卡片单一背景） -->
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 p-3 rounded-lg">
          <!-- CPU -->
